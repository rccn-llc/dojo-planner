...
describe('ArchiveMemberModal', () => {
  ...
  it('does not render when closed', async () => {
    await render(<ArchiveMemberModal {...baseProps} mode="archive" isOpen={false} />);

    expect(page.getByText('archive_title').elements()).toHaveLength(0);
  });

  it('shows the archive title + notice in archive mode', async () => {
    await render(<ArchiveMemberModal {...baseProps} mode="archive" />);
  ...
  it('shows the restore title + notice in restore mode', async () => {
    await render(<ArchiveMemberModal {...baseProps} mode="restore" />);
  ...
  it('calls member.remove and fires success when archiving', async () => {
    ...
    const onClose = vi.fn();
    await render(<ArchiveMemberModal {...baseProps} mode="archive" onSuccess={onSuccess} onClose={onClose} />);
  ...
  it('calls member.restore and fires success when restoring', async () => {
    const onSuccess = vi.fn();
    await render(<ArchiveMemberModal {...baseProps} mode="restore" onSuccess={onSuccess} />);
  ...
  it('surfaces an error and keeps the modal open on failure', async () => {
    ...
    await render(<ArchiveMemberModal {...baseProps} mode="archive" onClose={onClose} />);
