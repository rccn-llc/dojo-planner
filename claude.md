...
- **Data migrations that need the encryption key cannot be SQL.** `payment_provider_config_enc` holds AES-GCM ciphertext whose key lives in the app environment, so `src/scripts/migrateProviderConfigBlob.ts` performs the backfill and `b3-forward.sql` aborts rather than dropping the legacy columns until it has run. Order: deploy the reading code → run the script → apply the DDL. The script's `--reverse` flag unpacks the blob back into the columns for a rollback.
